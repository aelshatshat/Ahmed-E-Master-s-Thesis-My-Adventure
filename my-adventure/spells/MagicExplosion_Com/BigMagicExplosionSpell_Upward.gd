...
@onready var attack_area = $BigUpwardExplosion

func _ready():
	$AnimatedSprite2D.play("big_explosion")
	await $AnimatedSprite2D.animation_finished
	queue_free()


#func _on_big_upward_explosion_body_entered(body: Node2D) -> void:
	#if body.has_method("take_damage"):
		#body.take_damage(damage)

func _on_big_upward_explosion_area_entered(body: Area2D) -> void:
