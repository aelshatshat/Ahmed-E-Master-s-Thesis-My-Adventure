...
@onready var attack_area = $AreaExplosion

func _ready():
	$AnimatedSprite2D.play("explosion_attack2")
	await $AnimatedSprite2D.animation_finished
	queue_free()

#func _on_area_explosion_body_entered(body: Node2D) -> void:
	#if body.has_method("take_damage"):
		#body.take_damage(damage)
		
func _on_area_explosion_area_entered(body: Area2D) -> void:
